...
4. **Complexity**:
   ...
   - Space: O(1)."""
    ...
    boilerplate = {
        "python": "import sys\nimport re\n\ndef minCost(costs: list[list[int]]) -> int:\n    # User logic here\n    return 0\n\nif __name__ == '__main__':\n    raw_input = sys.stdin.read()\n    # Find all groups of 3 numbers in brackets [a, b, c]\n    rows = re.findall(r'\\[\\s*(-?\\\\d+)\\s*,\\s*(-?\\\\d+)\\s*,\\s*(-?\\\\d+)\\s*\\]', raw_input)\n    costs = [[int(r[0]), int(r[1]), int(r[2])] for r in rows]\n    if costs:\n        print(minCost(costs))",
        "cpp": "#include <iostream>\n#include <vector>\n#include <string>\n#include <regex>\n\nusing namespace std;\n\nint minCost(vector<vector<int>>& costs) {\n    // User logic here\n    return 0;\n}\n\nint main() {\n    string input, line;\n    while (getline(cin, line)) input += line + \" \";\n    \n    regex re_row(\"\\\\[\\\\s*(-?\\\\d+)\\\\s*,\\\\s*(-?\\\\d+)\\\\s*,\\\\s*(-?\\\\d+)\\\\s*\\\\]\");\n    auto b = sregex_iterator(input.begin(), input.end(), re_row);\n    auto e = sregex_iterator();\n    vector<vector<int>> costs;\n    for (auto i = b; i != e; ++i) {\n        costs.push_back({stoi((*i)[1]), stoi((*i)[2]), stoi((*i)[3])});\n    }\n    \n    if (!costs.empty()) {\n        cout << minCost(costs) << endl;\n    }\n    return 0;\n}",
        "java": "import java.util.*;\nimport java.util.regex.*;\n\npublic class Solution {\n    public int minCost(int[][] costs) {\n        // User logic here\n        return 0;\n    }\n\n    public static void main(String[] args) {\n        Scanner sc = new Scanner(System.in);\n        StringBuilder sb = new StringBuilder();\n        while (sc.hasNextLine()) sb.append(sc.nextLine()).append(\" \");\n        String input = sb.toString();\n        \n        List<int[]> list = new ArrayList<>();\n        Matcher m = Pattern.compile(\"\\\\[\\\\s*(-?\\\\d+)\\\\s*,\\\\s*(-?\\\\d+)\\\\s*,\\\\s*(-?\\\\d+)\\\\s*\\\\]\").matcher(input);\n        while (m.find()) {\n            list.add(new int[]{Integer.parseInt(m.group(1)), Integer.parseInt(m.group(2)), Integer.parseInt(m.group(3))});\n        }\n        \n        if (!list.isEmpty()) {\n            int[][] costs = list.toArray(new int[0][]);\n            System.out.println(new Solution().minCost(costs));\n        }\n    }\n}",
        "javascript": "const fs = require('fs');\n\nfunction minCost(costs) {\n    // User logic here\n    return 0;\n}\n\nconst input = fs.readFileSync(0, 'utf-8');\nconst matches = input.matchAll(/\\[\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\]/g);\nlet costs = [];\nfor (const m of matches) {\n    costs.push([parseInt(m[1]), parseInt(m[2]), parseInt(m[3])]);\n}\nif (costs.length > 0) {\n    console.log(minCost(costs));\n}",
        "c": "#include <stdio.h>\n#include <stdlib.h>\n#include <ctype.h>\n#include <string.h>\n\nint minCost(int** costs, int costsSize, int* costsColSize) {\n    // User logic here\n    return 0;\n}\n\nint main() {\n    static char buffer[1000000];\n    int bytes = fread(buffer, 1, sizeof(buffer)-1, stdin);\n    buffer[bytes] = '\\0';\n    \n    int** costs = (int**)malloc(1000 * sizeof(int*));\n    int* cols = (int*)malloc(1000 * sizeof(int));\n    int count = 0;\n    \n    char* p = buffer;\n    while (*p) {\n        if (*p == '[') {\n            p++;\n            while (*p && isspace(*p)) p++;\n            if (isdigit(*p) || *p == '-') {\n                costs[count] = (int*)malloc(3 * sizeof(int));\n                costs[count][0] = strtol(p, &p, 10);\n                while (*p && (*p != ',')) p++;\n                if (*p == ',') p++;\n                costs[count][1] = strtol(p, &p, 10);\n                while (*p && (*p != ',')) p++;\n                if (*p == ',') p++;\n                costs[count][2] = strtol(p, &p, 10);\n                cols[count] = 3;\n                count++;\n            }\n        } else p++;\n    }\n    \n    if (count > 0) {\n        printf(\"%d\\n\", minCost(costs, count, cols));\n    }\n    return 0;\n}"
    }
