...
class SnakeGame:
    ...
    boilerplate = {
        "python": (
            ...
            "    if len(input_data) >= 3:\n"
            "        parts = input_data[0].split()\n"
            "        w, h = int(parts[0]), int(parts[1])\n"
            "        food = json.loads(input_data[1].strip())\n"
            "        moves = json.loads(input_data[2].strip())\n"
            "        game = SnakeGame(w, h, food)\n"
            "        results = [game.move(m) for m in moves]\n"
        ...
        "cpp": (
            ...
            "        else if (c == ',' || c == ']') {\n"
            "            if (!val.empty()) { curr.push_back(stoi(val)); val = \"\"; }\n"
            "            if (c == ']' && !curr.empty()) { res.push_back(curr); curr.clear(); }\n"
            "        }\n"
            "    }\n"
            "    return res;\n"
            "}\n\n"
            ...
            "        int w = 0, h = 0;\n"
            "        size_t space = line1.find(' ');\n"
            "        if (space != string::npos) {\n"
            "            w = stoi(line1.substr(0, space));\n"
            "            h = stoi(line1.substr(space + 1));\n"
            "        }\n"
            "        vector<vector<int>> food = parse2DArray(line2);\n"
            "        vector<string> moves = parseStringArray(line3);\n"
            "        SnakeGame game(w, h, food);\n"
            "        for (size_t i = 0; i < moves.size(); i++) {\n"
            "            cout << game.move(moves[i]) << (i == moves.size() - 1 ? \"\" : \" \");\n"
            "        }\n"
            "        cout << endl;\n"
            "    }\n"
            "    return 0;\n"
        ...
        "java": (
            ...
            "            if (sc.hasNextLine()) {\n"
            "                String foodStr = sc.nextLine().trim();\n"
            "                List<int[]> foodList = new ArrayList<>();\n"
            "                String[] parts = foodStr.replace(\"[[\", \"\").replace(\"]]\", \"\").split(\"\\\\],\\\\s*\\\\[\");\n"
            "                for (String p : parts) {\n"
            "                    if (!p.isEmpty()) {\n"
            "                        String[] fParts = p.split(\",\");\n"
            "                        foodList.add(new int[]{Integer.parseInt(fParts[0].trim()), Integer.parseInt(fParts[1].trim())});\n"
            "                    }\n"
            "                }\n"
            "                int[][] food = foodList.toArray(new int[0][]);\n"
            "                if (sc.hasNextLine()) {\n"
            "                    String moveStr = sc.nextLine().trim();\n"
            "                    String[] moves = moveStr.replaceAll(\"[\\\\[\\\\]\\\\\\\"\\\\s]\", \"\").split(\",\");\n"
            "                    SnakeGame game = new SnakeGame(w, h, food);\n"
            "                    StringBuilder sb = new StringBuilder();\n"
            "                    for (int i = 0; i < moves.length; i++) {\n"
            "                        if (moves[i].isEmpty()) continue;\n"
            "                        sb.append(game.move(moves[i]));\n"
            "                        if (i + 1 < moves.length) sb.append(' ');\n"
            "                    }\n"
        ...
        "javascript": (
            ...
            "    if (input.length >= 3) {\n"
            "        const [w, h] = input[0].trim().split(/\\s+/).map(Number);\n"
            "        const food = JSON.parse(input[1].trim() || '[]');\n"
            "        const moves = JSON.parse(input[2].trim() || '[]');\n"
        ...
        "c": (
            ...
            "#include <stdlib.h>\n"
            "#include <ctype.h>\n\n"
            "int snakeMove(int width, int height, int** food, int foodSize,\n"
            "              char* direction, int* snakeR, int* snakeC,\n"
            "              int* snakeLen, int* foodIdx, int* score) {\n"
            "    // User logic here\n"
            "    return 0;\n"
            "}\n\n"
            "int main() {\n"
            ...
            "    char* line3 = NULL; size_t len3 = 0;\n"
            "    if (getline(&line1, &len1, stdin) != -1 && getline(&line2, &len2, stdin) != -1 && getline(&line3, &len3, stdin) != -1) {\n"
            "        int w = 0, h = 0;\n"
            "        sscanf(line1, \"%d %d\", &w, &h);\n"
            "        int foodGrid[100][2];\n"
            "        int fc = 0;\n"
            "        char* p = line2;\n"
            "        int curr[2]; int cIdx = 0;\n"
            "        while (*p) {\n"
            "            if (isdigit(*p) || *p == '-') {\n"
            "                curr[cIdx++] = strtol(p, &p, 10);\n"
            "            } else if (*p == ']') {\n"
            "                if (cIdx == 2) {\n"
            "                    foodGrid[fc][0] = curr[0];\n"
            "                    foodGrid[fc][1] = curr[1];\n"
            "                    fc++;\n"
            "                }\n"
            "                cIdx = 0;\n"
            "                p++;\n"
            ...
            "            p++;\n"
            "        }\n"
            "        int snakeR[100000] = {0}, snakeC[100000] = {0};\n"
            "        int snakeLen = 1, foodIdx = 0, score = 0;\n"
            "        int* fp[100];\n"
            "        for (int i = 0; i < fc; i++) fp[i] = foodGrid[i];\n"
            "        for (int i = 0; i < mc; i++) {\n"
            "            int res = snakeMove(w, h, fp, fc, moves[i], snakeR, snakeC, &snakeLen, &foodIdx, &score);\n"
            "            printf(\"%d%s\", res, (i + 1 < mc ? \" \" : \"\"));\n"
            "        }\n"
            "        printf(\"\\n\");\n"
            "    }\n"
            "    return 0;\n"
