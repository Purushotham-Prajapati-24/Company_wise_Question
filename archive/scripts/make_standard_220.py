...
3. **Complexity**:
   ...
   - Space Complexity: O(indexDiff) for the dictionary storing elements in the current window."""
    ...
    boilerplate = {
        "python": "import sys\n\ndef containsNearbyAlmostDuplicate(nums, indexDiff, valueDiff):\n    # User logic here\n    return False\n\nif __name__ == '__main__':\n    lines = sys.stdin.read().splitlines()\n    if len(lines) >= 3:\n        idxD = int(lines[0])\n        valD = int(lines[1])\n        nums = [int(x) for x in lines[2].split()]\n        print(\"true\" if containsNearbyAlmostDuplicate(nums, idxD, valD) else \"false\")",
